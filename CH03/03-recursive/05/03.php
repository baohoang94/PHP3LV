<?php
	...
	function cmsSelectbox($source, $name, $selected = 0, $style = null, $size = 0) {
		...
		foreach ($source as $key => $value) {
			...
			if ($value['level'] == 1) {
				$xhtml .= '<option id="'.$value['id'].'" '.$strSelected.'>+'.$value['name'].'</option>';
			} else {
				$name = str_repeat('&nbsp;', ($value['level']-1)*5) . '-' . $value['name'];
				$xhtml .= '<option id="'.$value['id'].'" '.$strSelected.'>'.$name.'</option>';
			}
	...
	echo cmsSelectbox($newArray, 'selectbox', 6);
?>
